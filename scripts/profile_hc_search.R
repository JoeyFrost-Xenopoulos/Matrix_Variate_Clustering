...
print(system.time({
  fit_serial <- matrix_variate_noise_fit(x_list, g = 2, select_noise_k = TRUE, use_parallel = FALSE, verbose = TRUE)
}))

cat("Parallel run timing (2 cores):\n")
print(system.time({
  fit_parallel <- matrix_variate_noise_fit(x_list, g = 2, select_noise_k = TRUE, use_parallel = TRUE, n_cores = 2, verbose = TRUE)
}))
